...
internal object TermGlyphContent {

    /** Terminal brand icon (blue→purple gradient, matches the tool-window stripe icon). */
    val TERMINAL_ICON: Icon = IconLoader.getIcon("/icons/term-glyph.svg", TermGlyphContent::class.java)

    /** Maps a docked Content → its TerminalBrowserPanel (used by the close-confirmation + repaint listeners). */
    val PANEL_KEY = Key.create<TerminalBrowserPanel>("termglyph.panel")
    ...
    /** True while a native split is in progress (set by TermGlyphSplitContentProvider.createContentCopy, cleared on the
    ...
    fun createContent(project: Project, disposable: Disposable, workDir: String): Content {
        ...
        // forbidden on EDT"). The shell is resolved inside TerminalSession from TermGlyph's own setting, falling
        // back to the platform default — see TerminalSession.shellPath.
        val panel = TerminalBrowserPanel(disposable, workDir, null)
        val title = assignSessionName(panel)
        val content = ContentFactory.getInstance().createContent(panel.component, title, false)
        content.icon = TERMINAL_ICON   // terminal icon on the tab, like the built-in terminal
        ...
        panel.onProcessChange = { processName ->
            ApplicationManager.getApplication().invokeLater {
                ...
                runCatching {
                    content.icon = if (processName != null) iconFor(processName) ?: TERMINAL_ICON else TERMINAL_ICON
    ...
    private fun loadIcon(path: String): Icon? = IconLoader.findIcon(path, TermGlyphContent::class.java)
