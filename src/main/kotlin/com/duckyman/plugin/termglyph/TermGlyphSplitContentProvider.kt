...
/**
 * Native side-by-side split for the TermGlyph tool window — **Split Right / Split Down**, Change Orientation,
 ...
 * `toolWindowId="TermGlyph"` in plugin.xml. The platform invokes `createContentCopy` when the user splits a
 ...
 */
class TermGlyphSplitContentProvider : ToolWindowSplitContentProvider {
    ...
    override fun createContentCopy(project: Project, content: Content): Content {
        ...
        TermGlyphContent.splitting = true
        com.intellij.openapi.application.ApplicationManager.getApplication().invokeLater { TermGlyphContent.splitting = false }
        ...
        val disposable = ToolWindowManager.getInstance(project).getToolWindow("TermGlyph")?.disposable
            ?: project
        ...
        return TermGlyphContent.createContent(project, disposable, workDir)
    }
}
